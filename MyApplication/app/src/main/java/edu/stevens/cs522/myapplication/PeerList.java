...
public class PeerList extends ListActivity {
    ...
    public boolean onOptionsItemSelected(MenuItem item){
        Intent addIntent;
        String _id, _name, _regId, _clientId;
        Cursor c;
        switch (item.getItemId()) {
            ...
            case (R.id.info):
                ...
                _id = c.getString(c.getColumnIndex(PeerContract.ID));
               /* _name = c.getString(c.getColumnIndex(PeerContract.NAME));
                _regId = c.getString(c.getColumnIndex(PeerContract.REGID));
                _clientId = c.getString(c.getColumnIndex(PeerContract.CLIENTID));*/
                addIntent = new Intent(getApplicationContext(), PeerInfo.class);
                addIntent.putExtra(PeerContract.ID, _id);
                /*addIntent.putExtra(PeerContract.NAME, _name);
                addIntent.putExtra(PeerContract.REGID, _regId);
                addIntent.putExtra(PeerContract.CLIENTID, _clientId);*/
                startActivity(addIntent);
                return true;
            case(R.id.cancel):
                finish();
